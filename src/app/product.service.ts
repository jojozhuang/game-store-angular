...
export class ProductService {
  ...
  //Create product
  createProduct(product: Product): Observable<any> { // eslint-disable-line
  ...
  //Update product
  updateProduct(product: Product): Observable<any> { // eslint-disable-line
    return this.http
      .put(`${this.apiUrl}/${product.id}`, product, { observe: 'response' })
      .pipe(map((success) => success.status));
  }
  //Delete product
  deleteProductById(pid: number): Observable<any> { // eslint-disable-line
    return this.http.delete(`${this.apiUrl}/${pid}`, { observe: 'response' }).pipe(map((success) => success.status));
  }
  //Upload image
  upload(fileToUpload: any): Observable<ResponseResult> { // eslint-disable-line
